...
def main():
    ...
    spreadsheet = {"properties": {"title": f"{businesstype} in {location}"}}
    spreadsheet = (
    ...
    driver.get(f"https://www.google.com/search?q={businesstype}%20in%20{location}")
    ...
    try:
        while True:
            ...
            for entry in results:
                ...
                time.sleep(1)
                try:
                    row.append(driver.find_element("xpath", "//h2/*").text)  # name
                except NoSuchElementException:
                    row.append("N/A")
                try:
                    row.append(
                        driver.find_element(  # website
                            "xpath", "//div[contains(text(), 'Website')]/.."
                        ).get_attribute("href")
                    )
                except NoSuchElementException:
                    row.append("N/A")
                ...
                try:
                    row.append(
                        driver.find_element(  # review count
                            "xpath",
                            "//div[@data-attrid='kc:/local:lu attribute list']/div/div/div/span[2]",
                        ).text
                    )
                except NoSuchElementException:
                    row.append("N/A")
                try:
                    row.append(
                        driver.find_element(  # category
                            "xpath",
                            "//div[@data-attrid='kc:/local:lu attribute list']/div[2]/div/span[last()]",
                        ).text
                    )
                except NoSuchElementException:
                    row.append("N/A")
                try:
                    row.append(
                        driver.find_element(  # phone
                            "xpath", "//span[contains(@aria-label,'Call phone')]"
                        ).text
                    )
                except NoSuchElementException:
                    row.append("N/A")
                try:
                    row.append(
                        driver.find_element(  # address
                            "xpath", "//*[contains(text(),'Address')]/../../*[2]"
                        ).text
                    )
                except NoSuchElementException:
                    row.append("N/A")
                ...
                row.append(
                    datetime.datetime.now().strftime(f"%Y-%m-%d %H:%M:%S")  # Timestamp
                )
